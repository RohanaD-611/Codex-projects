...
async function readQaItems(request, env) {
  const url = new URL("/assets/qa-data.json", request.url);
  const response = await env.ASSETS.fetch(new Request(url));
  if (!response.ok) {
    throw new Error("Unable to load qa-data.json.");
  }
  return response.json();
}

function buildKnowledgeBase(qaItems) {
  return qaItems
    .map((item) =>
      [
        `问题：${item.question}`,
...
export async function onRequestPost({ request, env }) {
  ...
  const baseUrl = env.DEEPSEEK_BASE_URL || DEEPSEEK_BASE_URL;
  ...
  try {
    ...
    const qaItems = await readQaItems(request, env);
