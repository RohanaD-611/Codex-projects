...
def main():
    ...
    preset_labels = [
        ("你在找什么样的公司和岗位？", "你在找什么样的公司和岗位？"),
        ("你的优势是什么？", "你的优势是什么？"),
        ("带来什么价值", "你能为我们带来什么？"),
    ]
    recommended_items = []
    recommended_labels = {}
    for match_text, label in preset_labels:
        item = next(
            (candidate for candidate in qa_items if match_text in candidate["question"]),
            None,
        )
        if item:
            recommended_items.append(item["id"])
            recommended_labels[item["id"]] = label

    app = f"""const qaItems = {json.dumps(qa_items, ensure_ascii=False, indent=2)};

const recommendedLabels = {json.dumps(recommended_labels, ensure_ascii=False, indent=2)};
const recommendedOrder = {json.dumps(recommended_items, ensure_ascii=False)};
